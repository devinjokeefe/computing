...
; The timer-int and button-int examples from lecture material for CSU11022 were used for this program.
...
Reset_Handler
	...
	;
	; Initialise timerCount in RAM
	;
	LDR	R4, =timerCount	; R4 = addr_timer_count
	LDR	R5, =0			; R5 = 0
	STR	R5, [R4]		; Memory.Word(addr_timer_count) = 0
...
Button_Handler
	...
	LDR	R4, =EXTINT			; Load the EXTINT register
	MOV	R5, #1				; Set EXTINT register to 1
	STRB	R5, [R4]		; Reset EINT0 interrupt

	LDR	R4, =timerCount		; addr_count = timerCount
	LDR	R5, [R4]			; tmp_timer_Count = Memory.Word(addr_count);
	
	LDR R6, =T0TC			; addr_time = timer0.time
	LDR R7, [R6]			; tmp_time = Memory.Word(addr_time)
	STR	R5, [R6]			; Memory.Word(addr_time) = tmp_timer_count;
	STR R7, [R4]			; Memory.Word(addr_count) = tmp_time
