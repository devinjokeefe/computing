...
doGet
	...
	CMP R0, #68 	; Check if the number is less than the ascii value of lowest possible input
	BLS fin			; If so, exit program as there is incorrect input
	CMP R0, #0x7a	; Check if the number is less than the ascii value of lowest possible input
	BHI fin			; If so, exit program as there is incorrect input
	
	SUB R0, R0, #0x20	; Conversion
